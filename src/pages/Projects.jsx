...
function VideoEmbed({ src, title, portrait = false }) {
  if (!src) return null
  return (
    <div style={{
      position: 'relative',
      paddingBottom: portrait ? '177.78%' : '56.25%',
      height: 0,
      overflow: 'hidden',
      borderRadius: '8px',
      background: '#000',
      maxWidth: portrait ? '340px' : '100%',
...
function ProjectModal({ project, onClose }) {
  ...
  const deckPreviewUrl = toPreview(project.deckUrl)
      ...
      <div className="modal-content">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          {[
            ...
            <div key={item.label}>
              <SectionLabel label={item.label} color={color} />
              <p style={{ color: '#F0F0EE', fontSize: '0.95rem', lineHeight: 1.7 }}>{item.text}</p>
            </div>
          ))}
          ...
          {project.teamNames && (
            <div>
              <SectionLabel label="Team" color={color} />
          ...
          {project.deckUrl && (
            <div>
              <SectionLabel label="Presentation Deck" color={color} />
              {/* Embedded viewer */}
              <div style={{
                position: 'relative',
                paddingBottom: '62%',
                ...
                <iframe
                  src={deckPreviewUrl}
                  title={`${project.campaignName} deck`}
                  style={{
              ...
              <a
                ...
                style={{
                  ...
                  fontSize: '0.82rem', fontWeight: 700, color: '#FFB020',
          ...
          <div>
            ...
            {project.billboardVideoUrl ? (
              ...
              <div style={{
                ...
                alignItems: 'center', justifyContent: 'center', gap: '8px', color: '#888',
              }}>
          ...
          {/* Platform link */}
          <div>
            <SectionLabel label="Campaign Platform" color={color} />
            {project.platformUrl ? (
              <a href={project.platformUrl} target="_blank" rel="noopener noreferrer"
                className="btn-cyan" style={{ display: 'inline-block' }}>
                Visit Platform →
              </a>
            ) : (
        ...
        {/* Vote CTA */}
        <div style={{
          marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #2A2A2E',
          display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center',
        }}>
          <Link to={`/vote?group=${project.id}`} className="btn-red" onClick={onClose}>
            Audience Vote
          </Link>
          <button onClick={onClose} className="btn-outline-white">Back to Projects</button>
        </div>
      </div>
    </div>
  )
    ...
    >
      ...
      {thumbUrl ? (
        <div style={{ position: 'relative', aspectRatio: '16/9', background: '#0D0D0F', overflow: 'hidden' }}>
          <img
            src={thumbUrl}
            alt={`${project.campaignName} deck cover`}
            style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
            onError={e => { e.currentTarget.parentElement.style.display = 'none' }}
          />
          {/* Play badge if has video */}
          {project.billboardVideoUrl && (
            <div style={{
              position: 'absolute', bottom: '8px', right: '8px',
              background: 'rgba(0,0,0,0.75)', borderRadius: '4px',
              padding: '3px 8px', fontSize: '0.65rem', fontWeight: 700,
              letterSpacing: '0.06em', color: '#fff', backdropFilter: 'blur(4px)',
            }}>
              ▶ VIDEO
            </div>
          )}
        ...
        /* Coloured placeholder if no deck */
        <div style={{ height: '6px', background: `linear-gradient(90deg, ${color}, ${color}80)` }} />
      )}
      ...
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '10px', flex: 1 }}>
        ...
        <div style={{
          ...
          textTransform: 'uppercase', color: '#00D4E8',
        }}>
          {groupLabel}
        </div>
        ...
        {/* Campaign name */}
        <h3 style={{
          fontWeight: 900, fontSize: 'clamp(1.05rem, 2vw, 1.25rem)', letterSpacing: '-0.01em',
          textTransform: 'uppercase', color: '#F0F0EE', lineHeight: 1.2, flex: 1,
        }}>
          {project.campaignName}
        </h3>

        {/* Hook */}
        <p style={{ color: '#888', fontStyle: 'italic', fontSize: '0.88rem', lineHeight: 1.6 }}>
          "{project.hook}"
        </p>
        ...
        <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid #2A2A2E' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Link
              ...
              style={{
                fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.06em',
                textTransform: 'uppercase', color: '#FF4422',
                padding: '6px 12px', border: '1px solid rgba(255,68,34,0.3)',
                borderRadius: '4px', textDecoration: 'none', transition: 'all 0.2s ease',
              }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,68,34,0.1)' }}
...
export default function Projects() {
  ...
  return (
    <main style={{ paddingTop: '64px' }}>
      {/* Hero */}
      <section style={{ background: '#0D0D0F', padding: '80px 0 60px', borderBottom: '1px solid #1C1C1E' }}>
        <div className="container">
          <div className="section-eyebrow">Trimester 2, AY 2025/26</div>
          <h1 style={{
            fontWeight: 900, fontSize: 'clamp(2.5rem, 7vw, 5rem)',
            letterSpacing: '-0.02em', textTransform: 'uppercase',
            color: '#F0F0EE', lineHeight: 1, marginBottom: '16px',
          }}>
            Nine<br /><span style={{ color: '#00D4E8' }}>Campaigns.</span>
          </h1>
          <p style={{ fontSize: '1.1rem', color: '#888', lineHeight: 1.8, maxWidth: '580px' }}>
      ...
      <section className="section">
        <div className="container">
            ...
            {filters.map(f => (
              <button
                ...
                style={{
                  ...
                  textTransform: 'uppercase', padding: '8px 16px', borderRadius: '100px',
                ...
                onMouseEnter={e => { if (activeFilter !== f.value) { e.currentTarget.style.borderColor = '#888'; e.currentTarget.style.color = '#F0F0EE' } }}
                onMouseLeave={e => { if (activeFilter !== f.value) { e.currentTarget.style.borderColor = '#2A2A2E'; e.currentTarget.style.color = '#888' } }}
              >
                {f.label}
              </button>
            ))}
          ...
          {/* Count */}
          <div style={{ color: '#888', fontSize: '0.85rem', marginBottom: '24px' }}>
